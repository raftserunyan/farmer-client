...
export const RateForm = ({
	...
	const loadRates = async () => {
		try {
			const rates = await HttpService.post('rating', {
	...
	const loadCurriculum = async () => {
		try {
			const { curriculum } = await HttpService.post(
	...
	const loadSubjectsOfProfession = async () => {
		const subjects = await loadProfessionSubjects({
			professionId: student.group.professionId,
			limit: 0,
		});
								...
								<FormLabelItem label='Գնահատականներ'>
									<S.CurriculumContainer>
										<S.CurriculumContainerHeader>
											{createArrayOfLength(
												student.group.profession?.yearsCount
											).map(year => {
												return (
													<S.YearContainer>
														<S.Course>
