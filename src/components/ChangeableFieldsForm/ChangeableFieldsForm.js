...
import {
	initialValues,
	getValidationSchema,
} from './ChangeableFieldsForm.config';
...
export const ChangeableFieldsForm = ({
	...
	const canSetFieldValue = (fieldName, component) => {
		if (!changeableColumns.hasOwnProperty(fieldName))
			return null;
			...
			>
				{({
					...
					const selectedValues = {
						commissariat: state.commissariats.list.find(
							el => el.id === values.commissariatId
						),
						citizenship: state.citizenships.list.find(
							el => el.id === values.citizenshipId
						),
						healthStatus: state.healthStatuses.list.find(
							el => el.id === values.healthStatusId
						),
						nationality: state.nationalities.list.find(
							el => el.id === values.nationalityId
						),
						profession: state.professions.list.find(
							el => el.id === values.professionId
						),
						privilege: state.privileges.list.find(
							el => el.id === values.privilegeId
						),
						status: state.statuses.list.find(
							el => el.id === values.statusId
						),
						group: state.groups.list.find(
							el => el.id === values.groupId
						),
						registrationRegion: state.regions.list.find(
							el => el.id === values.registrationRegionId
						),
						registrationCommunity:
							this?.registrationRegion?.communities.find(
								el =>
									el.id === values.registrationCommunityId
							),
						residentRegion: state.regions.list.find(
							el => el.id === values.residentRegionId
						),
						residentCommunity:
							this?.residentRegion?.communities.find(
								...
								<S.FormRow>
									{[
										...
										canSetFieldValue(
											...
											<Input
												value={
													values.acceptanceCommandNumber
												}
									...
									{[
										canSetFieldValue(
											...
											<FormLabelItem label='Գրանցման հասցե'>
												<Select
													value={
														selectedValues.registrationRegion
													}
												...
												<Select
													value={
														selectedValues.registrationCommunity
													}
													options={
														selectedValues
															.registrationRegion
										...
										canSetFieldValue(
											...
											<FormLabelItem label='Բնակության հասցե'>
												<Select
													...
													error={
														touched.residentRegionId &&
														errors.residentRegionId
													}
												/>
												<Select
													value={
														selectedValues.residentCommunity
													}
													options={
														selectedValues.residentRegion
															?.communities
