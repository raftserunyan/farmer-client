...
export const getRotationStudensList =
	...
	try {
		await HttpService.put(`student/${studentId}`, {
			...
			acceptanceCommandNumber:
				+parsedValues.acceptanceCommandNumber,
		});
...
export const createStudent = values => async dispatch => {
	try {
		const createdStudent = await HttpService.post(
			'student',
			{
				...values,
		...
		toast.error(
			`Առաջացավ խնդիր: ${ex?.response?.message?.join?.(
				' - '
			)}`
		);
	}
