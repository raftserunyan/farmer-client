...
export const deleteSubject =
	(ids, professionId) => async dispatch => {
		try {
...
export const editSubject =
	...
	async dispatch => {
		try {
			...
			if (showNotification)
				toast.success(
					'Գործողությունը հաջողությամբ կատարվեց'
				);
		} catch (ex) {
			toast.error(`Առաջացավ խնդիր: ${ex.message}`);
		}
...
export const createProfession =
	values => async dispatch => {
		try {
			const createdProfession = await HttpService.post(
				'profession',
				values
			);
