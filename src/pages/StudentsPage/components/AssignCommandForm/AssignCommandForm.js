...
export const AssignCommandForm = props => {
	...
	const onSubmit = values => {
		...
		if (
			!isObjectEmpty(values.changeableColumns) &&
			values.changeableColumns.contactNumbers
		) {
			const { contactNumbers } = values.changeableColumns;

			command.changeableColumns.contactNumbers =
	...
	return (
		<S.AssignCommandFormContainer>
			<S.FormHeaderContainer>
				<S.HeaderTitle>Կցագրել հրաման</S.HeaderTitle>
						...
						<S.FormContentContainer>
							<S.FormItemsList>
								<Input
									...
									onChange={val =>
										setFieldValue('commandNumber', val)
									}
									onEnter={handleSubmit}
									autoFocus
									error={
										touched.commandNumber &&
								...
								<DatePicker
									...
									onChange={val =>
										setFieldValue('affectDate', val)
									}
									error={
										touched.affectDate && errors.affectDate
									}
								/>
								<S.FormItem>
									<Select
										...
										onChange={val => {
											...
											setFieldValue(
												...
												commandsList.find(
													command =>
														command.id === val?.value
												)
											);
										}}
										error={
											touched.commandId && errors.commandId
										}
									/>
									{!isObjectEmpty(
										values.selectedCommand
											?.changeableColumns
									) && (
										<S.ChangeableColumnsButton
											title='Փոփոխելի դաշտեր'
											onClick={() =>
												showModal(ChangeableFieldsForm, {
													changeableColumns:
														values.changeableColumns ||
														values.selectedCommand
															.changeableColumns,
