...
export const RotationForm = ({
	...
	const [rotationStudents, setRotationStudents] =
		useState(null);
	...
	const onSubmit = async values => {
		const data = await HttpService.post('rotation', values);
		// setRotationStudents(data)
		const nonEditableStudents = data.data.filter(student =>
			[0, 1].includes(student.educationStatus)
		);

		const editableStudents = data.data.filter(
			student =>
				!nonEditableStudents
					.map(el => el.id)
					.includes(student.id)
	...
	if (!!rotationStudents) {
		for (const [
			ind,
			student,
		] of rotationStudents?.data?.entries()) {
			if (student.educationStatus === 1)
				selectedStudents.push(true);
			else if (
				ind < rotationStudents.additional.freePlacesCount &&
				[2, 3].includes(student.educationStatus)
			)
				selectedStudents.push(true);
			else selectedStudents.push(false);
		}
	}
	...
	return (
		<S.RotationFormContainer>
			<S.FormHeaderContainer>
				<S.HeaderTitle>Ռոտացիա</S.HeaderTitle>
					...
					return (
						<S.FormContentContainer>
							<S.FormItemsList>
								...
								<Select
									value={
										values.studentSemesters[1]
											? values.studentSemesters[1] / 2
											: null
									}
									options={createArrayOfLength(
										selectedProfession?.yearsCount
									)}
									placeholder='Կուրս'
									onChange={val => {
										setFieldValue(
											'studentSemesters',
											getCourseSemesters(val?.value)
										);
									}}
									error={
										touched.studentSemesters &&
										errors.studentSemesters
									}
								/>
								{selectedProfession && (
									<S.SemestersList>
										...
										<S.SemesterItems>
											{createArrayOfLength(
												selectedProfession?.yearsCount * 2
											).map(el => {
												return (
													<S.SemesterItem key={el}>
														<div>{el}</div>
														<Checkbox
															checked={values.semestersForCalculation.includes(
																el
															)}
															onChange={checked => {
																setFieldValue(
																	...
																	checked
																		? values.semestersForCalculation.concat(
																				el
																		  )
																		: values.semestersForCalculation.filter(
																				item => item !== el
																		  )
																);
															}}
							...
							{rotationStudents && (
								<Table
									...
									columns={tableColumns.rotationStudents(
										values.semestersForCalculation
									)}
